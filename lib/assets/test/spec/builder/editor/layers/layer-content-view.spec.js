...
describe('editor/layers/layer-content-view/layer-content-view', function () {
  ...
  var createViewFn = function (options) {
    ...
    var stateDefModel = new Backbone.Model({
      json: {
        context: 'map'
      }
    });

    stateDefModel.getContext = function () {
      return 'map';
    };

    stateDefModel.url = function () {
      return 'http://carto';
    };

    stateDefModel.updateState = function (state) {
      this.save({
        json: state
      });
    };
  ...
  describe('._onTogglerChanged', function () {
    ...
    it('should set context correctly', function () {
      ...
      expect(view._stateDefinitionModel.get('json').context).toEqual('map');
      view._togglerModel.set('active', true, { silent: true });
      view._onTogglerChanged();
      expect(view.model.get('context')).toEqual('table');
      expect(view._stateDefinitionModel.get('json').context).toEqual('table');
